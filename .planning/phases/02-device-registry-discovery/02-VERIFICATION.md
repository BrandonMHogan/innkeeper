...
**Test:** Load `/dashboard` against the live backend in an actual browser, confirm unknown devices render correctly, and exercise Register end-to-end.
**Status: RESOLVED.** Exercised live in this verification pass against the real `docker compose` stack (db/api/frontend/capture) with two synthetic mDNS identities seeded directly via `/api/capture/mdns` (substituting for the LAN traffic blocked by item #1 above). The dashboard rendered the unknown devices, and the user confirmed registering one through the actual browser UI now succeeds end-to-end.
**Bugs found and fixed during this UAT pass (none caught by the existing automated test suite, since it runs against SQLite, not Postgres):**
- **Frontend deploy bugs** (`frontend/Dockerfile`, `frontend/nginx.conf`, new `frontend/src/routes/+page.svelte`): the SPA shell wasn't reachable at all on first real deploy — a stale nginx default `index.html` masked the build output, `try_files` 403'd on the bare root path, the root route had no `+page.svelte` to redirect to `/login`/`/dashboard`, and the SPA shell HTML had no explicit `Cache-Control`, so a browser could keep serving a cached broken response after the server was fixed. All four fixed and verified.
- **Backend type mismatches** (`backend/src/models/*.py`, `backend/src/routes/capture.py`): `register_device` returned `500` on every attempt. Root causes: (1) all six timestamp columns were declared as timezone-naive `Mapped[datetime]` in the ORM despite every migration creating them as `TIMESTAMP WITH TIME ZONE`, which crashed asyncpg's encoder the moment a value read back from the DB was reused in an INSERT; (2) `Device.type`'s `SAEnum(DeviceType)` bound the Python enum member's `.name` ("LAPTOP") instead of `.value` ("laptop"), which doesn't match the actual Postgres enum labels. Both fixed; registration now returns `201` and was confirmed working by the user in the live browser.
