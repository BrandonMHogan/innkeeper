<script lang="ts">
  ...
  async function handleSubmit(event: SubmitEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        errorMessage = 'Could not save password. Check that the API is reachable and try again.';
        loading = false;
        return;
      }
      await goto('/login');
...
<svelte:head>
  <title>Innkeeper — First Run Setup</title>
</svelte:head>
...
>
  ...
  >
    ...
    <p style="font-size: 14px; font-weight: 400; line-height: 1.5; color: var(--color-muted); margin: 0 0 16px 0;">
      Set a dashboard password to get started.
    </p>
